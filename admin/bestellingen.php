            ...
            <div class="col-12">
                ...
                <form method="post" action="php/addtocart.php">
                    <ul class="list-group mb-3">
                        ...
                        $stmt = $conn->prepare("SELECT COUNT(*) FROM bestellingen");
                        $stmt->execute();
                        $stmt->bind_result($amountofbestellingen);
                        $stmt->fetch();
                        $stmt->close();

                        $stmt = $conn->prepare("SELECT id, data, datetime, status FROM bestellingen ORDER BY ID DESC");
                        $stmt->execute();
                        $stmt->bind_result($orderid, $data, $datetime, $status);
                        while ($stmt->fetch()) {
                        ...
                        for ($y = 0; $y < $amountofbestellingen; $y++) {
                            ...
                            for ($x = 0; $x < $countproducts; $x++) {
                                ...
                                echo '</div>';
                                echo '<span class="text-muted">€75<br>';
                                if ($statusarray[$y] == 1 && $x == 0) {
                                    echo "<a href='../php/versturen.php?id={$idarray[$y]}' class='btn btn-secondary btn-sm text-white'>Verwerk Gehele Bestelling ↓</a></span>";
                                }
                                echo '</li>';
                            }
                            echo "<br>";
